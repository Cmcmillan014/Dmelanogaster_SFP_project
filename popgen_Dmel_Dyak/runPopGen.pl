...
main: {
        ...
        foreach my $transcript (@gene_object){
            ...
            if ($include_outgroups_flag) {
                ...
                system ("./scripts/fastagrep.pl -X -f $transcript.list $CDS_file > $transcript.fa");
...
### This part will be temporarily edited to alow MK test without adding                 
            ...
            ### Don't add orthologous seqnuences to analysis. ########
                ...
                system ("./scripts/fastagrep.pl -X $transcript $CDS_file > $transcript.fa");
                system ("sed -i.bak '/>/ s/ line=/_/g' $transcript.fa");
                ...
                if ($include_samples_flag) {
                    system ("./scripts/fastagrep.pl -f $samples_file $transcript.fa > $transcript.trimmed.fa");
                    system ("rm $transcript.fa");
                    system ("mv $transcript.trimmed.fa $transcript.fa");
                }

                if ($exclude_samples_flag) {
                    system ("./scripts/fastagrep.pl -v -f $samples_file $transcript.fa > $transcript.trimmed.fa");
                    system ("rm $transcript.fa");
                    system ("mv $transcript.trimmed.fa $transcript.fa");
                }
            }
            ...
            if ($mk_test_flag) {
                ...
                if ($pol eq "pol") {
                    system ("./scripts/MK.pl -outfile $mk_tmp_dir/$transcript.MKout.tmp1.txt -pol pol -outgroup $outgroup1 -outgroup $outgroup2 -ingroup $ingroup -dir ./$mk_tmp_dir/");
                    system ("./scripts/MK_trim.pl $mk_tmp_dir/$transcript.MKout.tmp1.txt > $mk_tmp_dir/$transcript.MKout.tmp2.txt");
                    system ("sed -i 's/$transcript.fa/$transcript/g' $mk_tmp_dir/$transcript.MKout.tmp2.txt");    
                
                # run MK.pl with "unpol" option 
                } elsif ($pol eq "unpol") {
                    system ("./scripts/MK.pl -outfile $mk_tmp_dir/$transcript.MKout.tmp1.txt -pol unpol -outgroup $outgroup -ingroup $ingroup -dir ./$mk_tmp_dir/");
                    system ("./scripts/MK_trim.pl $mk_tmp_dir/$transcript.MKout.tmp1.txt > $mk_tmp_dir/$transcript.MKout.tmp2.txt");
                    system ("sed -i 's/$transcript.fa/$transcript/g' $mk_tmp_dir/$transcript.MKout.tmp2.txt"); 
                }
            ...
            # popgen stats
            if ($popgen_flag) {
                    system ("./scripts/tajimasD_calculator.pl $transcript.aln_dna.afa > $popgen_tmp_dir/$transcript.all.PopGenOut.txt");
                    system ("sed -i 's/^/$transcript\t/g' $popgen_tmp_dir/$transcript.all.PopGenOut.txt");
                if ($split_sites_flag) {
                    system ("./scripts/splitSynNonsyn.py $transcript.aln_dna.afa");
                    system ("./scripts/tajimasD_calculator.pl $transcript.nonSyn.fasta > $popgen_tmp_dir/$transcript.nonSyn.PopGenOut.txt");
                    system ("./scripts/tajimasD_calculator.pl $transcript.Syn.fasta > $popgen_tmp_dir/$transcript.Syn.PopGenOut.txt");
